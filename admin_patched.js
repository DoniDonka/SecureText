document.addEventListener("DOMContentLoaded", () => {
  ...
  async function deleteCollectionInBatches(colRef, batchSize = 450, label = "items") {
    let deleted = 0;
    while (true) {
      ...
      await batch.commit();
      deleted += snap.size;
      setStatus(`Deleted ${deleted} ${label}...`);
  ...
  function openClass(classId, className) {
    // avoid stacking listeners (saves quota)
    clearListeners();
    setStatus("");
  ...
  function wireClassControls() {
    const btnRules = $("forceRulesBtn");
    const btnLogout = $("forceLogoutBtn");
    const btnDelChat = $("deleteAllChatBtn");
    const btnDelAnn = $("deleteAllAnnouncementsBtn");

    if (btnRules) btnRules.onclick = async () => {
      if (!currentClassId) return;
      if (!confirm("Force everyone to re-accept the rules?")) return;
      setStatus("Sending rules re-accept command...");
      try {
        ...
        setStatus("✅ Rules re-accept triggered.");
      } catch (e) {
        console.error(e);
    ...
    if (btnLogout) btnLogout.onclick = async () => {
      ...
      setStatus("Sending force logout command...");
      try {
        await commandsDoc(currentClassId).set({
    ...
    if (btnDelChat) btnDelChat.onclick = async () => {
      if (!currentClassId) return;
      if (!confirm("Delete ALL chat messages in this class? This cannot be undone.")) return;
      setStatus("Deleting chat messages...");
      try {
        const count = await deleteCollectionInBatches(messagesCol(currentClassId), 450, "messages");
        setStatus(`✅ Deleted ${count} messages.`);
      } catch (e) {
        console.error(e);
        setStatus("❌ Failed to delete chat messages.");
      }
    };

    if (btnDelAnn) btnDelAnn.onclick = async () => {
      if (!currentClassId) return;
      if (!confirm("Delete ALL announcements in this class? This cannot be undone.")) return;
      setStatus("Deleting announcements...");
      try {
        const count = await deleteCollectionInBatches(announcementsCol(currentClassId), 450, "announcements");
        setStatus(`✅ Deleted ${count} announcements.`);
      } catch (e) {
        console.error(e);
