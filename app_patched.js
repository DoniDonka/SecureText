document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadChat(name, classId, userId) {
    ...
    // ===== admin commands (force rules / force logout) =====
    chatUnsubs.push(
      commandsDoc(classId).onSnapshot((doc) => {
        ...
        // 1) Force logout
        const remoteLogout = typeof data.logoutNonce === "number" ? data.logoutNonce : 0;
        const seenLogoutRaw = localStorage.getItem(logoutNonceSeenKey(classId));
        const seenLogout = seenLogoutRaw == null ? null : Number(seenLogoutRaw);
        if (seenLogout === null) {
          localStorage.setItem(logoutNonceSeenKey(classId), String(remoteLogout));
        } else if (remoteLogout > seenLogout) {
          try { localStorage.setItem(logoutNonceSeenKey(classId), String(remoteLogout)); } catch {}
          try { clearChatListeners(); } catch {}
          try { clearLS(); } catch {}
          location.reload();
          return;
        }

        // 2) Force rules re-accept
        const remoteRules = typeof data.rulesVersion === "number" ? data.rulesVersion : 0;
        const seenRulesRaw = localStorage.getItem(rulesVersionSeenKey(classId));
        const seenRules = seenRulesRaw == null ? null : Number(seenRulesRaw);
        if (seenRules === null) {
          localStorage.setItem(rulesVersionSeenKey(classId), String(remoteRules));
        } else if (remoteRules > seenRules) {
          try { localStorage.setItem(rulesVersionSeenKey(classId), String(remoteRules)); } catch {}
          try { localStorage.removeItem(rulesKey(classId, userId)); } catch {}
